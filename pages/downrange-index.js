...
<meta name="description" content="Precision ballistics calculator with multi-manufacturer bullet database, trajectory calculator, wind calls, and range card printing.">
...
nav{position:sticky;top:0;z-index:100;background:rgba(26,21,16,0.97);backdrop-filter:blur(12px);border-bottom:1px solid var(--border);padding:0 32px;display:flex;align-items:center;justify-content:space-between;height:60px}
.nav-brand{display:flex;align-items:center;gap:12px}
.nav-logo{width:36px;height:36px;border-radius:6px;overflow:hidden;flex-shrink:0}
...
.btn-secondary{padding:8px 16px;background:none;border:1px solid var(--border-bright);border-radius:5px;color:var(--wheat);font-family:'JetBrains Mono',monospace;font-size:10px;letter-spacing:0.08em;text-transform:uppercase;cursor:pointer;transition:all 0.2s}
.btn-secondary:hover{background:rgba(196,169,107,0.1)}
...
.bullet-card{background:var(--card);border:1px solid var(--border);border-radius:8px;padding:16px;transition:all 0.2s;position:relative}
...
.bullet-name{font-family:'Playfair Display',serif;font-size:15px;font-weight:700;color:var(--cream);margin-bottom:8px;line-height:1.3;padding-right:28px}
.bullet-specs{display:flex;gap:8px;flex-wrap:wrap}
...
.use-bullet-btn{flex:1;padding:6px 12px;background:none;border:1px solid var(--border-bright);border-radius:4px;color:var(--wheat);font-family:'JetBrains Mono',monospace;font-size:10px;text-transform:uppercase;cursor:pointer;transition:all 0.2s}
.use-bullet-btn:hover{background:rgba(196,169,107,0.1)}
.fav-btn{width:32px;height:32px;background:none;border:1px solid var(--border);border-radius:4px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.2s;font-size:14px;flex-shrink:0}
.fav-btn:hover{border-color:var(--wheat)}
.fav-btn.active{border-color:var(--amber);background:rgba(212,136,58,0.1)}
.fav-star{position:absolute;top:12px;right:12px;width:24px;height:24px;background:none;border:none;cursor:pointer;font-size:14px;opacity:0.4;transition:all 0.2s;line-height:1}
...
.results-toolbar{display:flex;gap:8px;margin-bottom:16px;justify-content:flex-end}
...
<nav>
  <div class="nav-brand">
    <div class="nav-logo"><img src="/icons/icon-96.png" alt="DownRange"></div>
    <div>
  ...
  <div class="nav-tabs">
    ...
    <button class="nav-tab" onclick="showPanel('favorites')">&#9733; Favorites</button>
...
<div class="app">
  ...
  <div id="panel-calculator" class="panel active">
    <div class="calc-grid">
      ...
      <div>
        <div id="results-toolbar" class="results-toolbar" style="display:none">
          <button class="btn-secondary" onclick="printResults()">&#128438; Print / Save PDF</button>
          <button class="btn-secondary" onclick="saveCSV()">&#8595; Export CSV</button>
        </div>
        <div id="results">
  ...
  <div id="panel-database" class="panel">
    <div class="db-header">
      ...
      <button class="btn-secondary" onclick="showPanel('favorites')">&#9733; View Favorites (<span id="fav-count">0</span>)</button>
    </div>
    <div class="filter-row" id="mfr-filters" style="margin-bottom:10px"></div>
  ...
  <div id="panel-favorites" class="panel">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;flex-wrap:wrap;gap:12px">
      <div>
        <div style="font-family:'Playfair Display',serif;font-size:22px;font-weight:700;color:var(--cream)">Favorites</div>
        <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--stone);letter-spacing:0.1em;text-transform:uppercase;margin-top:4px">Quick access to your saved bullets</div>
      </div>
      <button class="btn-secondary" onclick="showPanel('database')">+ Browse Database</button>
    </div>
    <div class="bullet-grid" id="fav-grid"></div>
    <div id="fav-empty" style="display:none">
      <div class="empty-state">
        <svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="1.5">
          <path d="M24 6l4.5 9 10 1.5-7.25 7 1.75 10L24 29l-9 4.5 1.75-10L9.5 16.5 19.5 15z"/>
        </svg>
        <div class="empty-title">No favorites yet</div>
        <div style="font-size:13px;margin-top:8px;color:#6b6358">Star bullets in the database to save them here</div>
      </div>
    </div>
  </div>
  ...
  <div id="panel-wind" class="panel">
    <div class="wind-grid">
      <div class="card">
        ...
        <button class="btn-secondary" style="width:100%;margin-top:16px" onclick="printWind()">&#128438; Print Wind Card</button>
      </div>
...
const BULLETS = [
  ...
  {mfr:"Berger",name:"6.5mm 130gr Hybrid OTM Tactical",cal:"6.5mm",wt:130,bc:0.274,bcType:"G7",dia:0.264,type:"Tactical"},
  {mfr:"Berger",name:"6.5mm 140gr Elite Hunter",cal:"6.5mm",wt:140,bc:0.289,bcType:"G7",dia:0.264,type:"Hunting"},
  {mfr:"Berger",name:"6.5mm 156gr Elite Hunter",cal:"6.5mm",wt:156,bc:0.319,bcType:"G7",dia:0.264,type:"Hunting"},
  {mfr:"Berger",name:"6mm 105gr Hybrid Target",cal:"6mm",wt:105,bc:0.271,bcType:"G7",dia:0.243,type:"Target"},
  {mfr:"Berger",name:"6mm 109gr LR Hybrid Target",cal:"6mm",wt:109,bc:0.290,bcType:"G7",dia:0.243,type:"Target"},
  {mfr:"Berger",name:"6mm 95gr Classic Hunter",cal:"6mm",wt:95,bc:0.243,bcType:"G7",dia:0.243,type:"Hunting"},
  {mfr:"Berger",name:"7mm 180gr Hybrid Target",cal:"7mm",wt:180,bc:0.337,bcType:"G7",dia:0.284,type:"Target"},
  {mfr:"Berger",name:"7mm 190gr LR Hybrid Target",cal:"7mm",wt:190,bc:0.371,bcType:"G7",dia:0.284,type:"Target"},
  {mfr:"Berger",name:"7mm 195gr Elite Hunter",cal:"7mm",wt:195,bc:0.391,bcType:"G7",dia:0.284,type:"Hunting"},
  {mfr:"Berger",name:"7mm 168gr Classic Hunter",cal:"7mm",wt:168,bc:0.319,bcType:"G7",dia:0.284,type:"Hunting"},
  {mfr:"Berger",name:"30 Cal 155gr Hybrid Target",cal:"30 Cal",wt:155,bc:0.237,bcType:"G7",dia:0.308,type:"Target"},
  ...
  {mfr:"Berger",name:"30 Cal 200gr Hybrid Target",cal:"30 Cal",wt:200,bc:0.315,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Berger",name:"30 Cal 215gr Hybrid Target",cal:"30 Cal",wt:215,bc:0.334,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Berger",name:"30 Cal 230gr Hybrid OTM Tactical",cal:"30 Cal",wt:230,bc:0.368,bcType:"G7",dia:0.308,type:"Tactical"},
  {mfr:"Berger",name:"30 Cal 175gr VLD Hunting",cal:"30 Cal",wt:175,bc:0.268,bcType:"G7",dia:0.308,type:"Hunting"},
  {mfr:"Berger",name:"30 Cal 185gr VLD Hunting",cal:"30 Cal",wt:185,bc:0.274,bcType:"G7",dia:0.308,type:"Hunting"},
  {mfr:"Berger",name:"338 Cal 250gr Hybrid OTM Tactical",cal:"338 Cal",wt:250,bc:0.368,bcType:"G7",dia:0.338,type:"Tactical"},
  {mfr:"Berger",name:"338 Cal 300gr Hybrid OTM Tactical",cal:"338 Cal",wt:300,bc:0.417,bcType:"G7",dia:0.338,type:"Tactical"},
  // HORNADY
  {mfr:"Hornady",name:"22 Cal 75gr ELD-M",cal:"22 Cal",wt:75,bc:0.230,bcType:"G7",dia:0.224,type:"Target"},
  {mfr:"Hornady",name:"22 Cal 80gr ELD-M",cal:"22 Cal",wt:80,bc:0.243,bcType:"G7",dia:0.224,type:"Target"},
  {mfr:"Hornady",name:"6mm 103gr ELD-X",cal:"6mm",wt:103,bc:0.256,bcType:"G7",dia:0.243,type:"Hunting"},
  {mfr:"Hornady",name:"6mm 108gr ELD-M",cal:"6mm",wt:108,bc:0.276,bcType:"G7",dia:0.243,type:"Target"},
  {mfr:"Hornady",name:"6.5mm 130gr ELD-M",cal:"6.5mm",wt:130,bc:0.281,bcType:"G7",dia:0.264,type:"Target"},
  ...
  {mfr:"Hornady",name:"6.5mm 120gr ELD-X",cal:"6.5mm",wt:120,bc:0.250,bcType:"G7",dia:0.264,type:"Hunting"},
  {mfr:"Hornady",name:"7mm 162gr ELD-X",cal:"7mm",wt:162,bc:0.315,bcType:"G7",dia:0.284,type:"Hunting"},
  {mfr:"Hornady",name:"7mm 175gr ELD-X",cal:"7mm",wt:175,bc:0.350,bcType:"G7",dia:0.284,type:"Hunting"},
  ...
  {mfr:"Hornady",name:"30 Cal 212gr ELD-X",cal:"30 Cal",wt:212,bc:0.336,bcType:"G7",dia:0.308,type:"Hunting"},
  {mfr:"Hornady",name:"338 Cal 285gr ELD-M",cal:"338 Cal",wt:285,bc:0.417,bcType:"G7",dia:0.338,type:"Target"},
  {mfr:"Hornady",name:"338 Cal 300gr ELD-X",cal:"338 Cal",wt:300,bc:0.430,bcType:"G7",dia:0.338,type:"Hunting"},
  // SIERRA
  {mfr:"Sierra",name:"22 Cal 77gr MatchKing",cal:"22 Cal",wt:77,bc:0.196,bcType:"G7",dia:0.224,type:"Target"},
  {mfr:"Sierra",name:"22 Cal 80gr MatchKing",cal:"22 Cal",wt:80,bc:0.220,bcType:"G7",dia:0.224,type:"Target"},
  {mfr:"Sierra",name:"6mm 107gr MatchKing",cal:"6mm",wt:107,bc:0.261,bcType:"G7",dia:0.243,type:"Target"},
  {mfr:"Sierra",name:"6mm 115gr MatchKing",cal:"6mm",wt:115,bc:0.297,bcType:"G7",dia:0.243,type:"Target"},
  {mfr:"Sierra",name:"6.5mm 123gr MatchKing",cal:"6.5mm",wt:123,bc:0.243,bcType:"G7",dia:0.264,type:"Target"},
  {mfr:"Sierra",name:"6.5mm 142gr MatchKing",cal:"6.5mm",wt:142,bc:0.289,bcType:"G7",dia:0.264,type:"Target"},
  {mfr:"Sierra",name:"6.5mm 150gr MatchKing",cal:"6.5mm",wt:150,bc:0.313,bcType:"G7",dia:0.264,type:"Target"},
  {mfr:"Sierra",name:"7mm 183gr MatchKing",cal:"7mm",wt:183,bc:0.356,bcType:"G7",dia:0.284,type:"Target"},
  {mfr:"Sierra",name:"7mm 197gr MatchKing",cal:"7mm",wt:197,bc:0.391,bcType:"G7",dia:0.284,type:"Target"},
  {mfr:"Sierra",name:"30 Cal 168gr MatchKing",cal:"30 Cal",wt:168,bc:0.243,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Sierra",name:"30 Cal 175gr MatchKing",cal:"30 Cal",wt:175,bc:0.264,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Sierra",name:"30 Cal 185gr MatchKing",cal:"30 Cal",wt:185,bc:0.287,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Sierra",name:"30 Cal 200gr MatchKing",cal:"30 Cal",wt:200,bc:0.310,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Sierra",name:"30 Cal 220gr MatchKing",cal:"30 Cal",wt:220,bc:0.350,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Sierra",name:"338 Cal 300gr MatchKing",cal:"338 Cal",wt:300,bc:0.390,bcType:"G7",dia:0.338,type:"Target"},
  // NOSLER
  {mfr:"Nosler",name:"6mm 90gr AccuBond",cal:"6mm",wt:90,bc:0.221,bcType:"G7",dia:0.243,type:"Hunting"},
  {mfr:"Nosler",name:"6mm 105gr RDF",cal:"6mm",wt:105,bc:0.264,bcType:"G7",dia:0.243,type:"Target"},
  {mfr:"Nosler",name:"6.5mm 129gr AccuBond LR",cal:"6.5mm",wt:129,bc:0.263,bcType:"G7",dia:0.264,type:"Hunting"},
  ...
  {mfr:"Nosler",name:"6.5mm 130gr RDF",cal:"6.5mm",wt:130,bc:0.271,bcType:"G7",dia:0.264,type:"Target"},
  {mfr:"Nosler",name:"7mm 168gr AccuBond LR",cal:"7mm",wt:168,bc:0.340,bcType:"G7",dia:0.284,type:"Hunting"},
  {mfr:"Nosler",name:"7mm 175gr RDF",cal:"7mm",wt:175,bc:0.349,bcType:"G7",dia:0.284,type:"Target"},
  {mfr:"Nosler",name:"30 Cal 168gr RDF",cal:"30 Cal",wt:168,bc:0.254,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Nosler",name:"30 Cal 175gr RDF",cal:"30 Cal",wt:175,bc:0.275,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Nosler",name:"30 Cal 210gr AccuBond LR",cal:"30 Cal",wt:210,bc:0.336,bcType:"G7",dia:0.308,type:"Hunting"},
  // FEDERAL / SIERRA
  {mfr:"Federal",name:"6.5mm 130gr Berger Hybrid",cal:"6.5mm",wt:130,bc:0.274,bcType:"G7",dia:0.264,type:"Target"},
  {mfr:"Federal",name:"6.5mm 140gr Berger Hybrid",cal:"6.5mm",wt:140,bc:0.301,bcType:"G7",dia:0.264,type:"Target"},
  {mfr:"Federal",name:"30 Cal 175gr Sierra MatchKing",cal:"30 Cal",wt:175,bc:0.264,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Federal",name:"30 Cal 185gr Berger Juggernaut",cal:"30 Cal",wt:185,bc:0.286,bcType:"G7",dia:0.308,type:"Target"},
  // BARNES
  {mfr:"Barnes",name:"6mm 85gr TSX",cal:"6mm",wt:85,bc:0.209,bcType:"G7",dia:0.243,type:"Hunting"},
  {mfr:"Barnes",name:"6.5mm 127gr LRX",cal:"6.5mm",wt:127,bc:0.257,bcType:"G7",dia:0.264,type:"Hunting"},
  {mfr:"Barnes",name:"6.5mm 130gr LRX",cal:"6.5mm",wt:130,bc:0.263,bcType:"G7",dia:0.264,type:"Hunting"},
  {mfr:"Barnes",name:"7mm 145gr LRX",cal:"7mm",wt:145,bc:0.295,bcType:"G7",dia:0.284,type:"Hunting"},
  {mfr:"Barnes",name:"7mm 168gr LRX",cal:"7mm",wt:168,bc:0.317,bcType:"G7",dia:0.284,type:"Hunting"},
  {mfr:"Barnes",name:"30 Cal 150gr TTSX",cal:"30 Cal",wt:150,bc:0.212,bcType:"G7",dia:0.308,type:"Hunting"},
  {mfr:"Barnes",name:"30 Cal 168gr TAC-X",cal:"30 Cal",wt:168,bc:0.245,bcType:"G7",dia:0.308,type:"Tactical"},
  {mfr:"Barnes",name:"30 Cal 175gr LRX",cal:"30 Cal",wt:175,bc:0.279,bcType:"G7",dia:0.308,type:"Hunting"},
  {mfr:"Barnes",name:"338 Cal 280gr LRX",cal:"338 Cal",wt:280,bc:0.385,bcType:"G7",dia:0.338,type:"Hunting"},
  // LAPUA
  {mfr:"Lapua",name:"22 Cal 69gr Scenar",cal:"22 Cal",wt:69,bc:0.178,bcType:"G7",dia:0.224,type:"Target"},
  {mfr:"Lapua",name:"6mm 103gr Scenar-L",cal:"6mm",wt:103,bc:0.247,bcType:"G7",dia:0.243,type:"Target"},
  {mfr:"Lapua",name:"6.5mm 100gr Scenar",cal:"6.5mm",wt:100,bc:0.196,bcType:"G7",dia:0.264,type:"Target"},
  {mfr:"Lapua",name:"6.5mm 123gr Scenar",cal:"6.5mm",wt:123,bc:0.243,bcType:"G7",dia:0.264,type:"Target"},
  {mfr:"Lapua",name:"6.5mm 136gr Scenar-L",cal:"6.5mm",wt:136,bc:0.276,bcType:"G7",dia:0.264,type:"Target"},
  {mfr:"Lapua",name:"6.5mm 139gr Scenar-L",cal:"6.5mm",wt:139,bc:0.279,bcType:"G7",dia:0.264,type:"Target"},
  {mfr:"Lapua",name:"7mm 150gr Scenar-L",cal:"7mm",wt:150,bc:0.287,bcType:"G7",dia:0.284,type:"Target"},
  {mfr:"Lapua",name:"7mm 170gr Scenar-L",cal:"7mm",wt:170,bc:0.337,bcType:"G7",dia:0.284,type:"Target"},
  {mfr:"Lapua",name:"30 Cal 155gr Scenar",cal:"30 Cal",wt:155,bc:0.235,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Lapua",name:"30 Cal 167gr Scenar",cal:"30 Cal",wt:167,bc:0.253,bcType:"G7",dia:0.308,type:"Target"},
  ...
  {mfr:"Lapua",name:"30 Cal 200gr Scenar-L",cal:"30 Cal",wt:200,bc:0.312,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Lapua",name:"338 Cal 250gr Scenar",cal:"338 Cal",wt:250,bc:0.326,bcType:"G7",dia:0.338,type:"Target"},
  {mfr:"Lapua",name:"338 Cal 300gr Scenar-L",cal:"338 Cal",wt:300,bc:0.417,bcType:"G7",dia:0.338,type:"Target"},
  // CUTTING EDGE
  {mfr:"Cutting Edge",name:"6mm 87gr Lazer",cal:"6mm",wt:87,bc:0.226,bcType:"G7",dia:0.243,type:"Hunting"},
  {mfr:"Cutting Edge",name:"6.5mm 123gr Lazer",cal:"6.5mm",wt:123,bc:0.261,bcType:"G7",dia:0.264,type:"Hunting"},
  {mfr:"Cutting Edge",name:"6.5mm 130gr MTAC",cal:"6.5mm",wt:130,bc:0.285,bcType:"G7",dia:0.264,type:"Tactical"},
  {mfr:"Cutting Edge",name:"6.5mm 140gr MTAC",cal:"6.5mm",wt:140,bc:0.305,bcType:"G7",dia:0.264,type:"Tactical"},
  {mfr:"Cutting Edge",name:"7mm 147gr Lazer",cal:"7mm",wt:147,bc:0.290,bcType:"G7",dia:0.284,type:"Hunting"},
  {mfr:"Cutting Edge",name:"7mm 162gr MTAC",cal:"7mm",wt:162,bc:0.332,bcType:"G7",dia:0.284,type:"Tactical"},
  {mfr:"Cutting Edge",name:"30 Cal 175gr MTAC",cal:"30 Cal",wt:175,bc:0.283,bcType:"G7",dia:0.308,type:"Tactical"},
  {mfr:"Cutting Edge",name:"30 Cal 185gr MTAC",cal:"30 Cal",wt:185,bc:0.310,bcType:"G7",dia:0.308,type:"Tactical"},
  {mfr:"Cutting Edge",name:"338 Cal 265gr Lazer",cal:"338 Cal",wt:265,bc:0.374,bcType:"G7",dia:0.338,type:"Hunting"},
  // HAMMER
  {mfr:"Hammer",name:"6mm 87gr Hunter",cal:"6mm",wt:87,bc:0.218,bcType:"G7",dia:0.243,type:"Hunting"},
  {mfr:"Hammer",name:"6.5mm 123gr Hunter",cal:"6.5mm",wt:123,bc:0.254,bcType:"G7",dia:0.264,type:"Hunting"},
  {mfr:"Hammer",name:"6.5mm 135gr Hunter",cal:"6.5mm",wt:135,bc:0.276,bcType:"G7",dia:0.264,type:"Hunting"},
  {mfr:"Hammer",name:"6.5mm 144gr Hunter",cal:"6.5mm",wt:144,bc:0.294,bcType:"G7",dia:0.264,type:"Hunting"},
  {mfr:"Hammer",name:"7mm 139gr Hunter",cal:"7mm",wt:139,bc:0.272,bcType:"G7",dia:0.284,type:"Hunting"},
  {mfr:"Hammer",name:"7mm 154gr Hunter",cal:"7mm",wt:154,bc:0.298,bcType:"G7",dia:0.284,type:"Hunting"},
  {mfr:"Hammer",name:"7mm 166gr Hunter",cal:"7mm",wt:166,bc:0.318,bcType:"G7",dia:0.284,type:"Hunting"},
  {mfr:"Hammer",name:"30 Cal 155gr Hunter",cal:"30 Cal",wt:155,bc:0.240,bcType:"G7",dia:0.308,type:"Hunting"},
  {mfr:"Hammer",name:"30 Cal 168gr Hunter",cal:"30 Cal",wt:168,bc:0.262,bcType:"G7",dia:0.308,type:"Hunting"},
  {mfr:"Hammer",name:"30 Cal 185gr Hunter",cal:"30 Cal",wt:185,bc:0.288,bcType:"G7",dia:0.308,type:"Hunting"},
  {mfr:"Hammer",name:"338 Cal 270gr Hunter",cal:"338 Cal",wt:270,bc:0.375,bcType:"G7",dia:0.338,type:"Hunting"},
  // PETERSON
  {mfr:"Peterson",name:"6.5mm 130gr Match",cal:"6.5mm",wt:130,bc:0.271,bcType:"G7",dia:0.264,type:"Target"},
  {mfr:"Peterson",name:"6.5mm 140gr Match",cal:"6.5mm",wt:140,bc:0.301,bcType:"G7",dia:0.264,type:"Target"},
  {mfr:"Peterson",name:"6.5mm 147gr Match",cal:"6.5mm",wt:147,bc:0.319,bcType:"G7",dia:0.264,type:"Target"},
  {mfr:"Peterson",name:"7mm 175gr Match",cal:"7mm",wt:175,bc:0.349,bcType:"G7",dia:0.284,type:"Target"},
  {mfr:"Peterson",name:"30 Cal 175gr Match",cal:"30 Cal",wt:175,bc:0.280,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Peterson",name:"30 Cal 185gr Match",cal:"30 Cal",wt:185,bc:0.296,bcType:"G7",dia:0.308,type:"Target"},
  {mfr:"Peterson",name:"30 Cal 200gr Match",cal:"30 Cal",wt:200,bc:0.315,bcType:"G7",dia:0.308,type:"Target"},
];

let activeMfr='All', activeCal='All', showFavsOnly=false;
let lastRows=[], lastParams={};
const g=id=>document.getElementById(id);
const gv=id=>{const el=g(id);return el?parseFloat(el.value)||0:0};
const gs=id=>{const el=g(id);return el?el.value:''};

// FAVORITES
function getFavs(){try{return JSON.parse(localStorage.getItem('dr_favs')||'[]');}catch{return[];}}
function saveFavs(f){localStorage.setItem('dr_favs',JSON.stringify(f));}
...
function calculate(){
  ...
  const params={bc:gv('bc-value'),bcType:gs('bc-type'),wt:gv('weight'),mv:gv('mv'),zeroYds:gv('zero'),sightHt:gv('sight-height'),tempF:gv('temp'),altFt:gv('altitude'),windMph:gv('wind-speed'),windClock:parseFloat(gs('wind-dir'))||3,startYd:gv('range-start'),stopYd:gv('range-stop'),stepYd:gv('range-step'),units};
  const rows=calculateTrajectory(params);
  if(!rows.length)return;
  lastRows=rows; lastParams=params;
  const ul=units==='moa'?'MOA':units==='mils'?'MILS':'IN',lr=rows[rows.length-1];
  g('results').innerHTML='<div class="stat-grid"><div class="stat-box"><div class="stat-val">'+params.mv+'</div><div class="stat-label">MV (fps)</div></div><div class="stat-box"><div class="stat-val">'+params.bc+'</div><div class="stat-label">BC ('+params.bcType+')</div></div><div class="stat-box"><div class="stat-val">'+lr.vel+'</div><div class="stat-label">Vel @ '+lr.range+'yds</div></div><div class="stat-box"><div class="stat-val">'+lr.energy+'</div><div class="stat-label">Energy @ '+lr.range+'yds</div></div></div><div class="card" style="padding:0;overflow:hidden"><table class="range-card-table"><thead><tr><th>Range (yds)</th><th>Drop ('+ul+')</th><th>Wind ('+ul+')</th><th>Vel (fps)</th><th>Energy (ft-lbf)</th><th>ToF (s)</th></tr></thead><tbody>'+rows.map(r=>'<tr><td class="'+(r.isZero?'zero':'')+'">'+r.range+'</td><td class="'+(r.drop>0.5?'neg':'zero')+'">'+r.dropDisp+'</td><td>'+r.driftDisp+'</td><td>'+r.vel+'</td><td>'+r.energy+'</td><td>'+r.time+'</td></tr>').join('')+'</tbody></table></div>';
  g('results-toolbar').style.display='flex';
}
...
function printResults(){
  ...
  const p=lastParams;
  const ul=p.units==='moa'?'MOA':p.units==='mils'?'MILS':'IN';
  const win=window.open('','_blank');
  win.document.write('<!DOCTYPE html><html><head><title>DownRange Range Card</title><style>body{font-family:monospace;padding:20px;color:#000;background:#fff}h1{font-size:18px;margin-bottom:4px}h2{font-size:13px;font-weight:normal;color:#555;margin-bottom:16px}table{width:100%;border-collapse:collapse;font-size:12px}th{background:#f0f0f0;padding:8px;text-align:right;border:1px solid #ddd;font-size:10px;text-transform:uppercase;letter-spacing:0.05em}th:first-child{text-align:left}td{padding:7px 8px;text-align:right;border:1px solid #eee}td:first-child{text-align:left;font-weight:bold}.info{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:16px}.info-box{border:1px solid #ddd;padding:8px;text-align:center}.info-val{font-size:18px;font-weight:bold}.info-lbl{font-size:9px;text-transform:uppercase;color:#888;letter-spacing:0.05em}@media print{body{padding:10px}}</style></head><body>');
  win.document.write('<h1>DownRange Range Card</h1><h2>'+p.bc+' BC ('+p.bcType+') · '+p.wt+'gr · MV '+p.mv+' fps · Zero '+p.zeroYds+'yds · '+new Date().toLocaleDateString()+'</h2>');
  const lr=lastRows[lastRows.length-1];
  win.document.write('<div class="info"><div class="info-box"><div class="info-val">'+p.mv+'</div><div class="info-lbl">MV (fps)</div></div><div class="info-box"><div class="info-val">'+p.bc+'</div><div class="info-lbl">BC ('+p.bcType+')</div></div><div class="info-box"><div class="info-val">'+lr.vel+'</div><div class="info-lbl">Vel @ '+lr.range+'yds</div></div><div class="info-box"><div class="info-val">'+lr.energy+'</div><div class="info-lbl">Energy @ '+lr.range+'yds</div></div></div>');
  win.document.write('<table><thead><tr><th>Range (yds)</th><th>Drop ('+ul+')</th><th>Wind ('+ul+')</th><th>Vel (fps)</th><th>Energy (ft-lbf)</th><th>ToF (s)</th></tr></thead><tbody>');
  lastRows.forEach(r=>{win.document.write('<tr><td>'+r.range+'</td><td>'+r.dropDisp+'</td><td>'+r.driftDisp+'</td><td>'+r.vel+'</td><td>'+r.energy+'</td><td>'+r.time+'</td></tr>');});
  win.document.write('</tbody></table><p style="margin-top:12px;font-size:10px;color:#aaa">Generated by DownRange · godownrange.com</p></body></html>');
  win.document.close();
...
function saveCSV(){
  ...
  const p=lastParams;
  const ul=p.units==='moa'?'MOA':p.units==='mils'?'MILS':'IN';
  let csv='Range (yds),Drop ('+ul+'),Wind ('+ul+'),Vel (fps),Energy (ft-lbf),ToF (s)\n';
  lastRows.forEach(r=>{csv+=r.range+','+r.dropDisp+','+r.driftDisp+','+r.vel+','+r.energy+','+r.time+'\n';});
  const blob=new Blob([csv],{type:'text/csv'});
  const a=document.createElement('a');
  a.href=URL.createObjectURL(blob);
  a.download='downrange-card-'+p.zeroYds+'yd-zero.csv';
  a.click();
}
...
function printWind(){
  ...
  const win=window.open('','_blank');
  win.document.write('<!DOCTYPE html><html><head><title>DownRange Wind Card</title><style>body{font-family:monospace;padding:20px}h1{font-size:18px;margin-bottom:4px}h2{font-size:13px;font-weight:normal;color:#555;margin-bottom:16px}table{border-collapse:collapse;font-size:12px;width:100%}th{background:#f0f0f0;padding:8px;border:1px solid #ddd;font-size:10px;text-transform:uppercase}td{padding:7px 8px;text-align:right;border:1px solid #eee}</style></head><body>');
  win.document.write('<h1>DownRange Wind Card</h1><h2>'+speed+' mph wind · '+clock+' o\'clock · '+range+' yds · MV '+mv+' fps · '+new Date().toLocaleDateString()+'</h2>');
  win.document.write('<table><thead><tr><th>Wind (mph)</th><th>Inches</th><th>MOA</th><th>MILS</th><th>1/4 MOA Clicks</th></tr></thead><tbody>');
  [5,10,15,20,25,30].forEach(s=>{
    const tof=range/(mv*0.8),di=s*0.44704*value*tof*(1/0.0254);
    const dm=di/(range*1.047/100),dmil=di*0.0254/(range*0.0009144);
    win.document.write('<tr><td>'+s+'</td><td>'+di.toFixed(1)+'</td><td>'+dm.toFixed(2)+'</td><td>'+dmil.toFixed(2)+'</td><td>'+Math.round(dm*4)+'</td></tr>');
  });
  win.document.write('</tbody></table><p style="margin-top:12px;font-size:10px;color:#aaa">Generated by DownRange · godownrange.com</p></body></html>');
  win.document.close();
  setTimeout(()=>win.print(),500);
...
function bulletCardHTML(b, idx, showUse=true){
  const fav=isFav(b.name);
  return '<div class="bullet-card"><button class="fav-star'+(fav?' active':'')+'" onclick="toggleFav(\''+b.name.replace(/'/g,"\\'")+'\')" title="'+(fav?'Remove from favorites':'Add to favorites')+'">'+(fav?'&#9733;':'&#9734;')+'</button><div class="bullet-mfr">'+b.mfr+'</div><div class="bullet-name">'+b.name+'</div><div class="bullet-specs"><span class="spec-tag">'+b.cal+'</span><span class="spec-tag">'+b.wt+' gr</span><span class="spec-tag bc">'+b.bcType+' '+b.bc+'</span><span class="spec-tag">'+b.type+'</span></div>'+(showUse?'<button class="use-bullet-btn" style="margin-top:10px;width:100%;" onclick="useBullet('+idx+')">Use in Calculator</button>':'')+'</div>';
}

function renderBullets(){
  const q=(g('db-search').value||'').toLowerCase();
  const filtered=BULLETS.map((b,i)=>({b,i})).filter(({b})=>(activeMfr==='All'||b.mfr===activeMfr)&&(activeCal==='All'||b.cal===activeCal)&&(!q||b.name.toLowerCase().includes(q)||b.mfr.toLowerCase().includes(q)));
  g('bullet-grid').innerHTML=filtered.map(({b,i})=>bulletCardHTML(b,i)).join('');
}

function renderFavs(){
  const favs=getFavs();
  const grid=g('fav-grid'),empty=g('fav-empty');
  if(!favs.length){grid.innerHTML='';empty.style.display='block';return;}
  empty.style.display='none';
  const favBullets=BULLETS.map((b,i)=>({b,i})).filter(({b})=>favs.includes(b.name));
  grid.innerHTML=favBullets.map(({b,i})=>bulletCardHTML(b,i)).join('');
}
...
function showPanel(id){
  ...
  document.querySelectorAll('.nav-tab').forEach(t=>{
    ...
    if((id==='calculator'&&txt.includes('calc'))||(id==='database'&&txt.includes('database'))||(id==='favorites'&&txt.includes('fav'))||(id==='wind'&&txt.includes('wind')))t.classList.add('active');
  });
